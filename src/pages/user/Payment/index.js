...
function Payment() {
    ...
    let isSuccessPayment = useSelector(
        (state) => state.user?.cart?.isSuccessPayment
    );
    let isSuccessGetMoMo = useSelector(
        (state) => state.user?.cart?.isSuccessGetMoMo
    );
    ...
    const handlePayment = (paymentMethod) => {
        ...
        console.log(transportInfo?.current);
        ...
        const order = {
            orderItem: currentUser
                ? cartProducts.map((item) => {
                      return {
                          productId: item.product._id,
                          size: item.size,
                          image: item.product.colors.find(
                              (i) => i.colorName === item.color
                          ).images[0],
        ...
        if (voucherIF) {
            totalMoney -= voucherIF.isPercent
                ? (voucherIF.voucherPrice / 100) * totalMoney
                : voucherIF.voucherPrice;
        }
            ...
            <div className={cx("container")}>
                ...
                <div className={cx("nav_left")}>
                    <div>
                        ...
                        >
                            ...
                            {currentUser && (
                                ...
                                >
                                    <img
                                        src={
                                            "https://www.coolmate.me/images/address_book_icon.svg"
                                        }
                                        alt=""
                                        style={{
                                            width: "16px",
                        ...
                        <TransportInfo
                            ref={transportInfo}
                            error={errAD}
                            props={currentUser ? addressSL : null}
                            userMail={currentUser ? currentUser.email : ""}
                        />
                    </div>
                ...
                {currentUser && (
                    <Modal visible={addressPopup} setModal={setAddressPopup}>
                        <div
                            className={cx("outer-address-popup")}
