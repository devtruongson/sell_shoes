    ...
    function onColorchange(e) {
        if (props.colors) {
            ...
            if (
                !listSize
                    .map((item) => item.name)
                    .includes(defaultSelection.size)
            ) {
                have = false;
            }
            have
                ? setDefaultSelection({
                      ...defaultSelection,
                      colorName: e.name,
                      image: props.colors.find(
                          (item) => item.colorName === e.name
                      )?.images[0],
                  })
                : setDefaultSelection({
                      size: listSize[0].name,
                      colorName: e.name,
                      image: props.colors.find(
                          (item) => item.colorName === e.name
                      )?.images[0],
                        ...
                        <div className={cx("selector")}>
                            <div style={{ width: "120px" }}>
                                <ComboBox
                                    listItems={colors}
                                    placeHolder={""}
                                    selectedItem={defaultSelection.colorName}
                                    type={"list-gray"}
                            ...
                            <div style={{ width: "120px" }}>
                                <ComboBox
                                    listItems={sizes}
                                    placeHolder={""}
                                    selectedItem={defaultSelection.size}
                                    type={"list-gray"}
                                    filterValueSelected={onSizeChange}
                                />
                            </div>
                        </div>
                        ...
                        >
                            ...
                            >
                                {new Intl.NumberFormat("vi-VN", {
                                    style: "currency",
                                    currency: "VND",
                                }).format(props.exportPrice)}
                            </del>
                        </div>
                        <div
                            className={cx("account-info__btn")}
                            onClick={() =>
                                handleItemToOrder(props, defaultSelection)
                            }
                        >
