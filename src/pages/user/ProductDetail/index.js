...
} from "~/api/product/get-product-by-type";
...
function ProductDetail() {
    ...
    const ButtonGroup = ({ next, previous, goToSlide, ...rest }) => {
        ...
        return (
            <>
                <div
                    onClick={() => previous()}
                    className={cx("btn-switch2", "left")}
                >
                    <FaArrowLeft className={cx("icon")} />
                </div>
                <div
                    onClick={() => next()}
                    className={cx("btn-switch2", "right")}
                >
    ...
    const handleClickBtnAddCart = () => {
        ...
        if (
            product.colors[indexColorActive].sizes[indexSizeActive].quantity ===
            0
        ) {
            setOutOfStock(true);
            return;
        ...
        const cartItem = {
            ...
            productPrice: product.discountPerc
                ? product.exportPrice * (1 - product.discountPerc / 100)
                : product.exportPrice,
            size: product.colors[indexColorActive].sizes[indexSizeActive]
                .sizeName,
        ...
        if (currentUser) {
            ...
            existItem
                ? increaseQuantityCartItem(
                      currentUser,
                      { ...existItem, quantity: quantityAddCart },
                      dispatch
                  )
    ...
    const getProductsByProductType = async () => {
        try {
            ...
            const res = await getListProductByType(product.productType);
    ...
    const getReviewsByProductId = async () => {
        try {
            const res = await axios.get(
                `${baseUrl}/api/reviews/getReviewsByProductId/${id}`
            );
    ...
    useEffect(() => {
        ...
        if (filterReview.star !== "all")
            nextStateReviews = nextStateReviews.filter(
                (item) => item.star === Number(filterReview.star)
            );
        if (filterReview.isHasImage !== "all") {
            if (filterReview.isHasImage)
                nextStateReviews = nextStateReviews.filter(
                    (item) => item.imagesRv && item.imagesRv.length > 0
                );
            else
                nextStateReviews = nextStateReviews.filter(
                    (item) => !item.imagesRv || item.imagesRv.length === 0
        ...
        if (filterReview.isResponsed !== "all")
            nextStateReviews = nextStateReviews.filter(
                (item) => item.isResponsed === filterReview.isResponsed
            );
    ...
    const hanleClickPlusQuantity = () => {
        ...
        setQuantityAddCart((prev) => {
            const quantityAvaiable =
                product.colors[indexColorActive].sizes[indexSizeActive]
                    .quantity;
            ...
            <div className={cx("body")}>
                <div className={cx("product-image-wrapper")}>
                    <ul className={cx("product-list-image")}>
                        {product.colors &&
                            product.colors[indexColorActive]?.images.map(
                                (item, index) => {
                                    return (
                                        <li key={index}>
                                            <div
                    ...
                    <div
                        ...
                        className={cx("btn-switch", "left")}
                    >
                        <FaAngleLeft />
                    </div>
                    <div
                        onClick={handleClickNextImage}
                        className={cx("btn-switch", "right")}
                    >
                        <FaAngleRight />
                    </div>
                </div>
                ...
                <div className={cx("section-info")}>
                        ...
                        {product.discountPerc ? (
                            <>
                                <span className={cx("price1")}>
                                    {formatMoney(
                                        product.exportPrice *
                                            (1 - product.discountPerc / 100)
                                    )}
                                </span>
                                <span className={cx("price2")}>
                                    {formatMoney(product.exportPrice)}
                                </span>
                            </>
                        ) : (
                            <>
                    ...
                    <div style={{ marginTop: "8px" }}>
                        <div>
                            ...
                            >
                                {product.colors &&
                                    product.colors[indexColorActive]?.colorName}
                            </span>
                        </div>
                        <div
                            style={{
                        ...
                        >
                            {product.colors?.map((item, index) => {
                                return (
                                    <div
                                        key={index}
                                        className={cx("img-color-wrapper", {
                                            active: index === indexColorActive,
                                        })}
                                    >
                                        <img
                                            onClick={() => {
                    ...
                    <div
                        style={{ marginTop: "8px" }}
                        className={cx("section-size", {
                            active: enableNotiSize,
                        })}
                    >
                        <div>
                            <span
                        ...
                        >
                            {product.colors &&
                                product.colors[indexColorActive]?.sizes.map(
                                    (item, index) => {
                                        return (
                                            <div
                                                onClick={() => {
                        ...
                        <div style={{ marginTop: "8px" }}>
                            <div>
                                ...
                                >
                                    ...
                                    <strong>
                                        {(product.colors &&
                                            product.colors[indexColorActive]
                                                ?.sizes[indexSizeActive]
                                                ?.quantity) ||
                                            0}
                                    </strong>
                                </span>
                        ...
                        {[...Array(Math.round(rate) || 0)].map(
                            (item, index) => {
                                return (
                                    <img
                                        key={index}
                                        src="https://www.coolmate.me/images/star-yellow.svg?8f4d13b24f276e8a788250b192548210"
                                    />
                                );
                            }
                        )}
                ...
                <div className={cx("reviews-rightside")}>
                    <div className={cx("reviews__filter")}>
                        <div className={cx("reviews__select")}>
                            <select
                                name="star"
                                onChange={handleChangeFilterReview}
                                style={{
                                    backgroundImage:
                                        "url(" + { FaAngleDown } + ")",
                                }}
                        ...
                        <div className={cx("reviews__select")}>
                            <select
                                name="isHasImage"
                                onChange={handleChangeFilterReview}
                                className={cx("reviews-filter-image")}
                            >
